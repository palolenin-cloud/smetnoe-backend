...
// --- Настройка сервера ---
// --- ОБНОВЛЕННЫЙ БЛОК НАСТРОЙКИ CORS ---

// Настраиваем CORS. Это "паспортный контроль" для нашего сервера.
// Он должен явно разрешить "въезд" запросам с нашего сайта на Vercel.
app.use(cors({
  origin: 'https://smetnoe-frontend.vercel.app', // Разрешаем запросы ТОЛЬКО с этого адреса
  methods: ['GET', 'POST'], // Разрешаем только те типы запросов, которые мы используем
}));
