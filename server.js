...
bot.on('callback_query', (query) => {
    ...
    if (query.data === 'buy_access') {
        ...
        // ИСПРАВЛЕНИЕ: Мы используем ваш РЕАЛЬНЫЙ публичный адрес!
...
// НОВОЕ: Эндпоинт-заглушка для проверки работоспособности сервера (GET /)
app.get('/', (req, res) => {
    // Возвращаем простой статус о том, что бэкенд активен
    res.status(200).send('Сервер Сметных Калькуляторов запущен. Ожидание POST-запросов на /api/calculate.');
});
...
app.get('/api/payment-success', (req, res) => {
    ...
    if (pendingPayments.has(paymentId) && pendingPayments.get(paymentId) === userId) {
        ...
        // Добавлено уточнение про повторное использование ссылки.
        res.redirect(`${redirectUrl}?error=Неверные данные платежа. Возможно, вы уже использовали эту ссылку.`);
    }
});
